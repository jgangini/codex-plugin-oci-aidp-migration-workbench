...
description: Recover, cancel, clean up, or monitor AIDP operations, including active job-run cancellation, isolated runtime reset, compute cleanup, and optional Slack notification through the existing local automation skill when the user explicitly requests it. Use when Codex needs to intervene in stuck or long-running AIDP flows, clean the environment before redeploying, or watch a live migration safely.
---
...
5. Use cleanup flows only after verifying the resource is not still the intended active target.
6. Trigger Slack or external notifications only when the user explicitly asks for them.
