...
description: Recover, cancel, clean up, or monitor AIDP operations, including active job-run cancellation, isolated runtime reset, compute cleanup, platform teardown checks, and optional Slack notification through the existing local automation skill when the user explicitly requests it. Use when Codex needs to intervene in stuck or long-running AIDP flows, clean the environment before redeploying, or watch a live migration safely.
---
...
5. Prefer `uv run codex-oci aidp platforms delete --manifest <path> --confirm-name <name>` for AIDP platform teardown once the wrapper exposes it; until then, record any OCI CLI base fallback as a plugin gap.
...
- Do not delete an AIDP platform until compute and storage cleanup are complete and the platform name has been explicitly confirmed.
